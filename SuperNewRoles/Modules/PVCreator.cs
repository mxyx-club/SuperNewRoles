...
public static class PVCreator
{
    ...
    public static void Start2()
    {
        ...
        Info(players.Count.ToString());
        index = new();
        ventindex = new();
        for (int i = 0; i < posData.Count; i++)
        {
            index[i] = -1;
            Info($"{i} : {VentPos[i] != null}");
    ...
    private static Dictionary<int, int> index = new();
    private static Dictionary<int, int> ventindex = new();
    private static Dictionary<int, Coroutine> coros = new();
    private static float UpdateTime;
    private const float UpdateTimeD = 0.003f;
    private static List<PlayerControl> players = new();
    private static readonly List<List<Vector2>> posData = new()
        {
    ...
    private static List<(double, int)> ventData = new();
    private static List<List<(double, int)>> VentPos
    {
        get
        {
    ...
    private static bool OldIsVent;
    private static List<float> NextVentTime;
        ...
        if (Is2)
        {
            ...
            if (UpdateTime <= 0)
            {
                ...
                for (int i = 0; i < posData.Count; i++)
                {
                    ...
                    {
                        Info(NextVentTime[playerindex].ToString());
                        Info(ventindex[playerindex].ToString());
