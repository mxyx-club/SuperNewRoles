...
public static class Recorder
{
    ...
    private static GameData.PlayerOutfit CopyOutfit(GameData.PlayerOutfit outfit)
    {
    ...
    public static IEnumerator SavePositions()
    {
        if (writer == null)
        {
            Info("writerがnullです", "Recorder:SavePositions");
            yield break;
        }
        if (PlayerPositions == null)
        {
            Info("PlayerPositionsがnullです", "Recorder:SavePositions");
            yield break;
        }
    ...
    public static void OnEndGame(GameOverReason reason)
    {
        ...
        if (ReplayManager.IsReplayMode || !ReplayManager.IsRecording) return;
        Info("Start-Save-2");
        Info(writer.BaseStream.Length.ToString());
        Info(writer.BaseStream.Position.ToString());
        writer.Write(PlayerPositions.Count);
        Dictionary<byte, List<Vector2>> playerpositions = new(PlayerPositions);
        PlayerPositions = new();
        foreach (var data in playerpositions)
        ...
        {
            writer.Write((byte)action.GetActionId());
            Info(index.ToString() + "開始:" + action.GetActionId().ToString() + ":" + writer.BaseStream.Position.ToString());
            action.WriteReplayFile(writer);
            Info(index.ToString() + "終了:" + action.GetActionId().ToString() + ":" + writer.BaseStream.Position.ToString());
            index++;
        }
        Info(writer.BaseStream.Length.ToString());
        Info(writer.BaseStream.Position.ToString());
