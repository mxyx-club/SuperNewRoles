...
internal class AwakeMeetingPatch
{
    ...
    {
        UObject.Destroy(__instance.meetingContents.FindChild("PhoneUI").FindChild("UI_Icon_Battery").gameObject);
    }
}
...
internal class CastVotePatch
{
    ...
    public static bool Prefix(byte srcPlayerId, byte suspectPlayerId, MeetingHud __instance)
    {
        ...
        else // 無効票であれば,
        {
            ...
            Info($"{srcPlayer.name}({srcPlayerRole}) の 投票を無効化しました。 (投票先 : {(suspectPlayer != null ? $"{suspectPlayer.name}({suspectPlayer.GetRole()})" : "投票者無し")}", "Vote Void"); // ログに記載し,
            return false; // 無効化する。
        }
    }
}
...
internal class CheckForEndVotingPatch
{
    ...
    private static PlayerControl ChangeNameExiledPlayer;
    ...
    public static bool Prefix(MeetingHud __instance)
    {
        try
        {
            ...
            {
                Info($"金庫破りが追放ガードの条件を満たしましていました", "Safecracker Exiled Guard");
                Info($"金庫破りが追放ガードの回数(減らす前) : {(Safecracker.ExiledGuardCount.ContainsKey(exiledPlayer.Object.PlayerId) ? Safecracker.ExiledGuardCount[exiledPlayer.Object.PlayerId] : Safecracker.SafecrackerMaxExiledGuardCount.GetInt())}回", "Safecracker Exiled Guard");
                ...
                Info($"金庫破りが追放ガードの回数(減らした後) : {Safecracker.ExiledGuardCount[exiledPlayer.PlayerId]}回", "Safecracker Exiled Guard");
                exiledPlayer = null;
            }
...
internal class MeetingHudStartPatch
{
    public static void Postfix(MeetingHud __instance)
    {
        Info("会議開始時の処理 開始", "MeetingHudStartPatch");
        ...
        if (PlayerControl.LocalPlayer.IsRole(RoleId.Werewolf) && CachedPlayer.LocalPlayer.IsAlive() && !RoleClass.Werewolf.IsShooted)
        {
            CreateMeetingButton(__instance, "WerewolfKillButton", (int i, MeetingHud __instance) =>
            {
                if (RoleClass.Werewolf.IsShooted || CachedPlayer.LocalPlayer.IsDead() || !Mode.Werewolf.Main.IsUseButton())
                {
                    foreach (PlayerVoteArea state in __instance.playerStates)
                    {
                        if (state.transform.FindChild("WerewolfKillButton") != null)
                            UObject.Destroy(state.transform.FindChild("WerewolfKillButton").gameObject);
                    }
                    return;
                }
                ...
                if (Knight.GuardedPlayers.Contains((byte)i))
                {
                    ...
                    var Guard = UObject.Instantiate(FastDestroyableSingleton<RoleManager>.Instance.protectAnim, player.transform);
                    Guard.Play(player, null, player.cosmetics.FlipX, RoleEffectAnimation.SoundType.Global);
                    __instance.playerStates.ForEach(x => { if (x.transform.FindChild("WerewolfKillButton") != null) UObject.Destroy(x.transform.FindChild("WerewolfKillButton").gameObject); });
                    return;
                }
        ...
        Info("会議開始時の処理 終了", "MeetingHudStartPatch");
    }
...
public static class AnonymousVotes
{
    ...
    public static bool GetAnonymousVotes(this PlayerControl player)
    {
        ...
        // 公開投票ならログを出力
        if (!isClosed) Info($"公開投票 : {player.name}, (role = {player.GetRole()}, IsDead() = {player.IsDead()})", "OpenVotes");
        ...
        if (Instance)
        {
            foreach (PlayerVoteArea player in Instance.playerStates)
            {
                ...
                else
                {
                    ...
                    Error($"プレイヤーコントロールを取得できませんでした。 プレイヤー名 : {player.NameText.text}", "LightAndDarkerText");
