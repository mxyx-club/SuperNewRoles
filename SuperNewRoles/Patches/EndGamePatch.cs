...
public static class OnGameEndPatch
{
    ...
    public static void Prefix([HarmonyArgument(0)] ref EndGameResult endGameResult)
    {
        ...
        if (!ReplayManager.IsReplayMode && ConfigRoles.IsSendAnalytics.Value && !ConfigRoles.DebugMode.Value)
        {
            try
            {
    ...
    private static void ProcessReplaceWin(ref HashSet<GameData.PlayerInfo> winners, GameOverReason gameOverReason, ref WinCondition winCondition, out bool allowAdditionalWins)
    {
