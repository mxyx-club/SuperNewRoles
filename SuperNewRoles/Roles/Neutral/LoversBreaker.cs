...
{
    //ここにコードを書きこんでください
    private static bool IsViewd;
    private static bool IsNotSetted;
    public static void LateUpdate()
    {
        ...
        IsNotSetted = true;
        if (IsViewd) return;
        if ((AmongUsClient.Instance != null && AmongUsClient.Instance.GameState is InnerNetClient.GameStates.Started or InnerNetClient.GameStates.Ended) || SceneManager.GetActiveScene().name == "EndGame") return;
        if (DateTime.UtcNow >= new DateTime(2022, 12, 23, 12, 0, 0))
        {
            IsViewd = true;
            GenericPopup Popup = UObject.Instantiate(DiscordManager.Instance.discordPopup, Camera.main.transform);
            ...
            TextMeshPro Title = UObject.Instantiate(Popup.TextAreaTMP, Popup.transform);
            Title.text = "メリークリスマス！";
