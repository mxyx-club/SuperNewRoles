...
public static class CustomHatManager
{
    ...
    internal static string CosmeticDir = Path.Combine(Paths.GameRootPath, "SuperNewRoles");
    internal static string CustomHatsDir => Path.Combine(CosmeticDir, "CustomHats");
    internal static string RepositoryUrl => "https://github.com/SuperNewRoles/SuperNewCosmetics".GithubUrl();
    ...
    {
        Loader = SuperNewRolesPlugin.Instance.AddComponent<HatsLoader>();
    }
    ...
    {
        var texture = ModHelpers.LoadTextureFromDisk(Path.Combine(CustomHatsDir, path)) ?? ModHelpers.LoadTextureFromResources(path);
    ...
    {
        var filePath = Path.Combine(CustomHatsDir, resFile);
