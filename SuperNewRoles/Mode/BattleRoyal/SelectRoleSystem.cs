...
internal class IntroCutsceneOnDestroyPatch
{
    public static void Postfix(IntroCutscene __instance)
    {
        ...
        {
            SelectRoleSystem.OnEndIntro(); Info("StartOnEndIntro");
        }
    }
}
...
internal class GameDataSerializePatch
{
    ...
    {
        if (AmongUsClient.Instance is null || AmongUsClient.Instance.GameState != InnerNetClient.GameStates.Started || !ModeHandler.IsMode(ModeId.BattleRoyal) || Is || !Main.IsIntroEnded)
        {
            Is = false;
            __result = true;
            return true;
    ...
    public static void TeamOnlyChat()
    {
        foreach (BattleTeam team in BattleTeam.BattleTeams)
        {
            ...
            foreach (PlayerControl player in PlayerControl.AllPlayerControls)
            {
                ...
                Info($"{player.GetDefaultName()} : {player.Data.IsDead} : {!team.IsTeam(player)}");
            }
            Info("ーー開始ーー");
